...
@implementation simple_projectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_simple_project);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any simple_project patches in JS before a context is loaded... nice for 
...
{
	// create simple_project global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_simple_project = [[simple_projectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_simple_project];
	
	NSString *_simple_projectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _simple_projectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_simple_project valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.simple_project.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
